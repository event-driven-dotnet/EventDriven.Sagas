    ...
    ResultDispatchingSagaCommandHandler<CreateOrderSaga, Commands.CreateOrder, OrderState>
{
    ...
    public override async Task HandleCommandAsync(Commands.CreateOrder command)
    {
        try
        {
            ...
            if (addedOrder != null)
                await DispatchCommandResultAsync(addedOrder.State, false, command.SagaId, command.Entity);
            else
                await DispatchCommandResultAsync(OrderState.Initial, true, command.SagaId, command.Entity);
        }
